  ...
  return (
    <div ref={ref} className="relative z-[60]">
      <button
        ...
        className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/10 hover:bg-white/20 transition-colors text-lips-green-dark dark:text-white text-[10px] sm:text-xs font-medium"
        aria-label={t.prayer.changeRegion}
      >
        <MapPin className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-lips-gold flex-shrink-0" />
        <span>{selectedLabel}</span>
        <ChevronDown className={`h-2.5 w-2.5 sm:h-3 sm:w-3 text-lips-green-dark/60 dark:text-white/60 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      ...
      {open && (
        <motion.div
          ...
          className="fixed z-[9999] hidden sm:block bg-[#0f3d22] border border-white/20 rounded-xl shadow-2xl shadow-black/50 w-48"
          style={{
            top: btnRef.current
              ...
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-lg text-xs transition-colors flex items-center gap-2 ${
                  r.key === selected
                    ...
                    : 'text-lips-green-dark dark:text-white hover:bg-white/10'
                }`}
              >
      ...
      {open && (
        <>
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-lips-green/10 dark:border-white/10">
              <h3 className="text-lips-green-dark dark:text-white font-bold text-sm">{t.prayer.chooseRegion}</h3>
              <button onClick={() => setOpen(false)} className="p-1 rounded-lg hover:bg-white/10 transition-colors">
                <X className="h-5 w-5 text-lips-green-dark dark:text-white" />
              </button>
            </div>
...
export default function PrayerTimesWidget() {
  ...
  if (!mounted || !prayerData) {
    return (
      <div className="bg-background/90 backdrop-blur-xl text-lips-green-dark border-b border-lips-green/10 transition-colors duration-500">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3">
          <div className="flex items-center justify-center gap-3 text-xs text-lips-green-dark/70 dark:text-lips-green-dark/70 dark:text-white/70">
            <Clock className="h-4 w-4 text-lips-green dark:text-lips-gold animate-pulse" />
  ...
  return (
    <div className="bg-background/90 backdrop-blur-xl text-lips-green-dark w-full border-b border-lips-green/10 relative z-50 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 min-w-0">
        ...
        <div className="py-2.5 sm:hidden">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1 px-2 py-1 bg-lips-green/5 dark:bg-white/15 rounded-lg flex-shrink-0">
              <NextIcon className="h-3 w-3 text-lips-green-dark dark:text-white flex-shrink-0" />
              <div className="text-center">
                <div className="text-[9px] text-lips-green-dark/70 dark:text-white/70 leading-tight">{t.prayer.next}</div>
                <div className="font-mono text-[11px] font-bold text-lips-green-dark dark:text-white leading-tight">{countdown}</div>
              </div>
            </div>
            ...
            <button
              ...
              className="p-1.5 rounded-lg hover:bg-lips-green/5 dark:bg-white/15 transition-colors touch-manipulation flex-shrink-0"
              aria-label={expanded ? t.prayer.collapse : t.prayer.seeAllPrayers}
            >
              {expanded ? <ChevronUp className="h-4 w-4 text-lips-green-dark dark:text-white" /> : <ChevronDown className="h-4 w-4 text-lips-green-dark dark:text-white" />}
            </button>
          </div>
          ...
          <div className="flex items-center justify-between mt-1.5 gap-2">
            <Link href="/agenda" className="flex items-center gap-1.5 min-w-0 overflow-hidden">
              <span className="text-lips-green-dark dark:text-white text-[10px] font-semibold truncate">{isRTL ? hijriDisplay.arabic : hijriDisplay.french}</span>
            </Link>
                      ...
                      >
                        ...
                        <div className="min-w-0 overflow-hidden">
                          <div className={`text-[10px] leading-tight truncate font-semibold ${
                            isCurrent ? 'text-[#0a2e18]' : 'text-lips-green-dark dark:text-white'
                          }`}>
                            {getPrayerName(prayer.key)}
                          </div>
                          <div className={`font-mono text-[11px] leading-tight font-bold ${
                            isCurrent ? 'text-[#0a2e18]/80' : 'text-lips-green-dark dark:text-white'
                          }`}>
                            {time}
                          </div>
                        </div>
        ...
        <div className="hidden sm:block py-2.5 lg:py-3">
          ...
          <div className="flex items-center justify-between gap-2 sm:gap-3 mb-2.5 min-w-0">
            ...
            >
              ...
              <span className="text-lips-green-dark/70 dark:text-white/70 text-[10px] sm:text-xs truncate hidden sm:inline">{gregorianDate}</span>
              <span className="text-white/30 flex-shrink-0 hidden sm:inline">|</span>
              <span className="font-bold text-lips-green-dark dark:text-white text-xs sm:text-sm truncate">{isRTL ? hijriDisplay.arabic : hijriDisplay.french}</span>
            </Link>
            <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
              <div className="flex items-center gap-1 px-1.5 sm:px-2 py-1 bg-white/10 rounded-lg">
                <NextIcon className="h-3 w-3 text-lips-green-dark dark:text-white" />
                <span className="text-[8px] sm:text-[9px] text-lips-green-dark/70 dark:text-white/70">{t.prayer.next}</span>
                <span className="font-mono text-[10px] sm:text-xs font-bold text-lips-green-dark dark:text-white">{countdown}</span>
              </div>
              <RegionSelector selected={region} onChange={handleRegionChange} t={t} isRTL={isRTL} />
            </div>
          </div>
                ...
                <div
                  ...
                  className={`flex items-center justify-center gap-1.5 px-1.5 py-1.5 lg:px-2.5 rounded-lg transition-all overflow-hidden ${
                    isCurrent
                      ...
                      : 'bg-white/10 hover:bg-lips-green/5 dark:bg-white/15'
                  }`}
                >
                  <Icon className={`h-3.5 w-3.5 flex-shrink-0 ${isCurrent ? 'text-[#0a2e18]' : prayer.color}`} />
                  <div className="min-w-0 overflow-hidden">
